...
#            print(distlist)
#            print(outdens)
            return tf.constant([np.nan]*tf.size(atomcount).numpy())
...
#        pairsum = tf.linalg.matmul(outpair, self.dummyvec)
        ...
        tf.stack(paireng)
        tf.stack(embeng)
#        print(structcnt_np, tf.shape(paireng), tf.shape(embeng))
    ...
    def create_plot(self, threshold=0):
        ...
        pvalues = np.arange(-8.0, 8.0, 0.01, dtype=np.float32)
        tf_rvalues = tf.Variable(rvalues)
        tf_pvalues = tf.Variable(pvalues)
        rcnt = rvalues.size
        pcnt = pvalues.size
#        print(rvalues.shape)
#        print(pvalues.shape)
        outpair = self.pairsym(tf.reshape(tf_rvalues, shape=(rcnt,1))).numpy().reshape(-1)
        outdens = self.denssym(tf.reshape(tf_rvalues, shape=(rcnt,1))).numpy().reshape(-1)
        outemb = self.embsym(tf.reshape(tf_pvalues, shape=(pcnt,1))).numpy().reshape(-1)
        with open("pair_plot.dat", "w") as outfile:
            for r, e in zip(rvalues, outpair):
                outfile.write("%s %s\n"%(r,e))
        with open("dens_plot.dat", "w") as outfile:
            for r, e in zip(rvalues, outdens):
                outfile.write("%s %s\n"%(r,e))
        with open("emb_plot.dat", "w") as outfile:
            for p, e in zip(pvalues, outemb):
                outfile.write("%s %s\n"%(p,e))
    #----------------------------------------------------------------------------
