...
def sym_pp_hetrogeneous(W_list, funcs, var_names, threshold=0.01):
    """Pretty print the hidden layers (not the last layer) of the symbolic regression network
    ...
    for idx, W in enumerate(W_list):
        W = filter_mat(sym.Matrix(W), threshold=threshold)
        n_double = functions.count_double(funcs[idx])
        layer_funcs = [func.sp for func in funcs[idx]]
        expr = expr * W
        expr = apply_activation(expr, layer_funcs, n_double=n_double)
    # expr = expr * W_list[-1]
#    r = sym.symbols('r')
...
def hetrogeneous_network(weights, funcs, var_names, threshold=0.01):
    """Pretty print the entire symbolic regression network with hetrogeneous activation functions in each layer.

    Arguments:
        weights: list of weight matrices for the entire network
        funcs:  list of list of lambda functions using sympy.
        var_names: list of strings for names of variables
        threshold: threshold for filtering expression. set to 0 for no filtering.
    ...
    expr = sym_pp_hetrogeneous(weights[:-1], funcs, var_names, threshold=threshold)
    expr = last_pp(expr, weights[-1])
    expr = expr[0, 0]
    return expr
